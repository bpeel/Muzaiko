...
if ($jaro != 0 && $monato != 0 && $tago != 0) {
	$query = "SELECT TIME_FORMAT(komenchoro, '%H:%i'), TIME_FORMAT(finhoro, '%H:%i'), skizo, id FROM programero, elsendo WHERE programero.id = elsendo.programero_id AND YEAR(dato) = $jaro AND MONTH(dato) = $monato AND DAY(dato) = $tago ORDER BY komenchoro ASC";
	...
	while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
          printf('<li><a href="/programdetaloj?id=%s">' .
                 '%s&ndash;%s UTC</a>: %s</li>',
                 $row[3], $row[0], $row[1],
                 format_programero(htmlspecialchars(stripslashes($row[2]))));
	}
